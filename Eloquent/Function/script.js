...
/* Each script has an array of character code ranges associated with it. 
 * So given a character code, we could use a function characterScript to find the corresponding script (if any):
 * characterScript(121)); → {name: "Latin", …}
 * 
 */
function characterScript(code, property){
    for(let script of SCRIPTS){
        let result = script.ranges.some( ([lowerBound, upperBound]) => {
...
console.log(characterScript(72263, 'name'));

function countBy(str, property, func){
  let result = [];
  for(let character of str){
    let value = func(character);
    if(value){
      let index = result.findIndex( obj => obj[property] == value);
      if(index == -1)
        result.push({[property]: value, count: 1});
      else  
        result[index].count++;
    }
  }
...
function findLanguage(str){
  let scripts = countBy(str, 'name', char =>{
    let nameScript = characterScript(char.codePointAt(), 'name');
    if(nameScript)
      return nameScript;
    else  
      return null;
  ...
  let total = scripts.reduce((total, item) => {
  ...
  let percentage =  scripts.map( item => {
    return `${item.name}: ${Math.round((item.count / total) * 10000)/100}%`;
  }).join(', ');
  return percentage;
}
